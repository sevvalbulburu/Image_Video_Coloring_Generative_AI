...
def load_dataset(p):
    ...
    train_dataset_l = tf.keras.preprocessing.image_dataset_from_directory(
        WORKDIR+p, image_size=(IMAGE_SIZE, IMAGE_SIZE), batch_size=BATCH_SIZE, shuffle=False, label_mode=None
    )
...
train_dataset = load_dataset('village_dataset_independent/train/rgb/')
test_dataset = load_dataset('village_dataset_independent/test/rgb/')
...
def generator(): # autoencoder generator model
...
def generate_images(model, test_input, tar):
    ...
    for i in range(BATCH_SIZE):
        grayscale_lab = np.dstack((test_input[i], np.zeros((IMAGE_SIZE, IMAGE_SIZE, 2))))
        grayscale_rgb = lab2rgb(grayscale_lab)
        gray = (grayscale_rgb * 255).astype(np.uint8)

        orig_lab = np.dstack((test_input[i], tar[i] * 128))
        ...
        predicted_lab = np.dstack((test_input[i], prediction[i] * 128))
...
generator_independent = generator()
discriminator_independent = discriminator()
train(generator_independent, discriminator_independent,\
    {'name': 'adam', 'learning_rate': 0.0002, 'beta_1': 0.5}, {'name': 'adam', 'learning_rate': 0.0002, 'beta_1': 0.5}, \
        WORKDIR+'village_island_independent.csv', skip_training=True)
...
if False:
    ...
    for input_image, target in test_dataset:
        count += 1
        cv2.imwrite(csv_file[:-4]+str(count)+'.png', generate_images(generator, input_image, target))
...
if False:
    ...
    for input_image, target in test_dataset:
        count += 1
        cv2.imwrite(WORKDIR+'hababam'+str(count)+'.png', generate_image(generator_independent, input_image[0], target[0]))
